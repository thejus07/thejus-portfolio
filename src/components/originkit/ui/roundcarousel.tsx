...
export default function RoundCarousel({
  ...
  const onPointerUp = (e: React.PointerEvent) => {
    ...
    if (!dragRef.current.moved) {
      const card = document.elementFromPoint(e.clientX, e.clientY)?.closest<HTMLElement>('[data-round-carousel-item]');
      const index = Number(card?.dataset.roundCarouselItem);
      if (Number.isInteger(index)) items[index]?.onClick?.();
    }
            ...
            return (
              <div
                ...
                style={{
                  ...
                  transformStyle: "preserve-3d",
                  cursor: img.onClick ? "pointer" : undefined,
                }}
                role={img.onClick ? "button" : undefined}
                tabIndex={img.onClick ? 0 : undefined}
                data-round-carousel-item={i}
                onKeyDown={(event) => {
                  if (img.onClick && (event.key === "Enter" || event.key === " ")) img.onClick();
                }}
              >
                <div
                  style={{
                ...
                <div
                  style={{
                    ...
                    backgroundImage: src ? `url("${src}")` : undefined,
                    filter: `brightness(${Math.max(innerDim, 8) / 10})`,
                  }}
                />
              </div>
            );
